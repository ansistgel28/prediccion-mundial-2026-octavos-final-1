...
ARCHIVO_RESUMEN = OUT_DIR / "prediccion_octavos_a_cuartos.csv"
ARCHIVO_TOP10 = OUT_DIR / "top10_marcadores_octavos.csv"
ARCHIVO_TARJETAS = OUT_DIR / "prediccion_tarjetas_octavos.csv"
ARCHIVO_CUARTOS = OUT_DIR / "clasificados_cuartos.csv"
IMAGEN_PANEL = OUT_DIR / "panel_octavos_marcadores_tarjetas.png"
...
def validar_archivos():
    if not ARCHIVO_FIXTURES_OCTAVOS_MODELO.exists():
        raise FileNotFoundError(
            "No existe data/fixtures_octavos_modelo.csv. "
...
def cargar_datos():
    ...
    fixtures = pd.read_csv(ARCHIVO_FIXTURES_OCTAVOS_MODELO)
    fixtures.columns = fixtures.columns.str.strip()

    for col in VARIABLES:
        if col not in fixtures.columns:
            raise ValueError("Falta columna en fixtures_octavos_modelo.csv: " + col)
...
def construir_cuartos(df_resumen):
    cuartos = df_resumen[[
        "match_id",
        "date",
        "home_team",
    ...
    cuartos = cuartos.rename(columns={
        "match_id": "match_id_octavos",
        "final_winner": "team"
    })

    cuartos["classification_type"] = "CLASIFICADO_A_CUARTOS"
...
def graficar_panel(df_resumen, df_tarjetas):
    ...
    fig.suptitle(
        "Mundial 2026 - Octavos: Poisson + Dixon-Coles + tarjetas",
        fontsize=18,
        fontweight="bold"
    )
...
def main():
    ...
    cuartos = construir_cuartos(df_resumen)
    ...
    cuartos.to_csv(ARCHIVO_CUARTOS, index=False, encoding="utf-8")

    print()
    print("========================================")
    print("RESUMEN OCTAVOS A CUARTOS")
    print("========================================")
    print(df_resumen[[
        "match_id",
        "home_team",
    ...
    imprimir_detalle_partido(df_resumen, df_top10, df_tarjetas, 89)
    imprimir_detalle_partido(df_resumen, df_top10, df_tarjetas, 90)
